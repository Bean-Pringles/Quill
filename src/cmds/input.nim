...
proc inputIRGenerator*(
    ...
    if prompt.len >= 2 and (prompt[0] == '"' or prompt[0] == '\''):
    ...
    if target in ["exe", "ir", "zip"]:
        ...
        when defined(windows):
            ...
            let promptLen = prompt.len + 2
            
            # Generate prompt string, UNIQUE buffer, and bytes read tracker
            globalDecl &= "@inputPrompt" & $inputBufferCounter & " = global [" & $promptLen & " x i8] c\"" & prompt & "\\0A\\00\"\n"
            ...
            let promptLen = prompt.len + 1  # +1 for newline
            
            # Generate prompt string, UNIQUE buffer, and bytes read global
            globalDecl &= "@inputPrompt" & $inputBufferCounter & " = private unnamed_addr constant [" & $promptLen & " x i8] c\"" & prompt & "\\0A\"\n"
            globalDecl &= bufferGlobal & " = global [64 x i8] zeroinitializer\n"
        ...
        var batchCode = "set /p " & inputVarName & "=" & prompt
        inc inputBufferCounter
        ...
        let inputVarName = "input_var" & $inputBufferCounter
        var pythonCode = inputVarName & " = input(\"" & prompt & "\")"
        inc inputBufferCounter
