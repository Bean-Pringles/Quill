...
proc generateIR*(node: Node, commandsCalled: seq[string], commandNum: int): (string, seq[string], int) =
  ...
  if node.isCall:
    if registry.irGenerators.hasKey(node.commandName):
      ...
      return registry.irGenerators[node.commandName](argStrings, mutableCommandsCalled, commandNum)
    else:
