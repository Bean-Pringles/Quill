...
for file in walkDir(cmdsDir):
    if (file.kind == pcFile) and (not file.path.endsWith("commands.nim")) and (not file.path.endsWith("global.nim")) and (file.path.endsWith(".nim")):
...
if entries.len == 0:
    ...
    for e in entries:
        content &= "  registerIRGenerator(\"" & e.registerKey & "\", " &
                e.procName & ")\n"
